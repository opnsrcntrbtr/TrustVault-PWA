...
- **CSP parity update:** The allowed CDN origins (`WEBLLM_MODEL_ORIGINS` in `src/config/securityHeaders.ts` — `huggingface.co`, `cdn-lfs*.huggingface.co`, `raw.githubusercontent.com`) are added to `connect-src` and mirrored verbatim in `vercel.json`, enforced by the existing drift-guard parity test (`src/config/__tests__/securityHeaders.test.ts`). This allowlist is **provisional**: it is based on the known MLC/Hugging Face hosting pattern, not yet confirmed against a real device's Network tab. Task 11 (manual Android on-device verification, tracked in `TEST_STATUS.md`) requires recording the exact observed origins and trimming/extending this list to match before the feature is considered fully verified.
