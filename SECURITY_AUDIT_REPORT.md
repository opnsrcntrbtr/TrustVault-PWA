...
- **On-device outcome + hardening (Task 11, 2026-06-21):** On the test device (Android 10 / Adreno 6xx) WebGPU, Android gating, the Settings download UI, and the full weight download all verified working; the CSP allowlist was reconciled to zero violations (above). **Inference itself could not be exercised on this unit** — WebLLM loses the WebGPU device during engine warm-up (before any tokens), reproduced identically on both a 1B and a 0.5B model with `context_window_size` capped at 2048, i.e. a hard Android-10/Adreno-driver limitation (GPU process killed during shader compilation), not a code defect; the feature is opt-in + capability-gated so such devices simply should not enable it. Hardening added in response: a Tiny-tier model (`Qwen2.5-0.5B-Instruct-q4f16_1-MLC`, ~380MB) for best low-end compatibility (`webllmModels.ts`); `createEngine()` caps `context_window_size` at 2048 to shrink the warm-up footprint (`webllmProvider.ts`); and WebGPU device-loss is now detected, the engine state reset (no wedge — a retry re-creates), and surfaced as one clean error instead of the raw cascade (TDD-covered in `webllmProvider.test.ts`; UI already renders a clean "Could not generate an explanation." message + retry). Token streaming and airplane-mode offline inference remain to be confirmed on a device whose GPU driver sustains WebLLM init.
