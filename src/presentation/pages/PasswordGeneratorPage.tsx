...
export default function PasswordGeneratorPage() {
  ...
  // On-device AI strength explanation (opt-in, off by default)
      ...
      <Box sx={{ maxWidth: 800, mx: 'auto', p: 3 }}>
        ...
        <Card elevation={2} sx={{ mb: 3 }}>
          <CardContent>
            ...
            {/* On-device AI strength explanation (only when enabled + available) */}
            {ai.enabled && (
              <Box sx={{ mt: 1 }}>
                <Button
                  size="small"
                  variant="outlined"
                  disabled={ai.loading}
                  onClick={() => { void ai.explain({ strength, entropyBits: entropy }); }}
                >
                ...
                {ai.insight && <StrengthInsightCard insight={ai.insight} />}
                {!ai.insight && ai.rawText && (
                  <Box sx={{ mt: 1 }}>
                    <Typography variant="body2" color="text.secondary">{ai.rawText}</Typography>
