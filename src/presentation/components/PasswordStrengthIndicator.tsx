...
import { STRENGTH_SYSTEM_PROMPT } from '@/core/ai/strengthExplain';
...
export default function PasswordStrengthIndicator({
  ...
  const strengthInsight = useAiStrengthExplain();
  ...
  return (
    <Box sx={{ width: '100%', mt: 1 }}>
      ...
      {allowAiExplanation && aiAllowed && strengthInsight.enabled && (
        <Box sx={{ mt: 1 }}>
          {!expanded ? (
            <Button
              size="small"
              variant="outlined"
              disabled={strengthInsight.loading}
              onClick={() => {
                // Approximate entropy from score for AI input
                const entropyBits = Math.max(0, analysisResult.score);
                setExpanded(true);
                void strengthInsight.explain({ strength: analysisResult.strength, entropyBits });
              }}
            >
              {strengthInsight.loading ? 'AI is thinking…' : 'Explain with AI'}
            </Button>
          ) : (
            <>
              {strengthInsight.loading && (
                <Typography variant="caption" color="text.secondary" display="block" sx={{ mt: 0.5 }}>
                  AI is thinking…
                </Typography>
              )}
              {strengthInsight.insight && <StrengthInsightCard insight={strengthInsight.insight} />}
              {!strengthInsight.insight && strengthInsight.rawText && (
                <Box sx={{ mt: 1 }}>
                  <Typography variant="body2" color="text.secondary">{strengthInsight.rawText}</Typography>
                  <Typography variant="caption" color="text.disabled" display="block" sx={{ mt: 0.5 }}>
                    Generated by on-device AI.
                  </Typography>
                </Box>
              )}
              {strengthInsight.error && (
