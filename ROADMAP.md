...
| 5 | **Encrypted Import/Export & Recovery** ✅ shipped — “I can export my vault to a password-protected `.tvault` file and re-import/merge safely.” | Closed the high-risk plaintext export gap and enables disaster recovery. | `CredentialRepositoryImpl.exportAll/importFromJson`, `core/crypto/exportEncryption.ts` (AES-256-GCM + PBKDF2 600k), `ExportDialog.tsx`, `ImportDialog.tsx`, `SettingsPage`. | Crypto-roundtrip unit tests (correct/wrong password, duplicate handling) and `import-export.test.tsx` integration spec landed. Remaining: extend coverage to merge-conflict resolution. |
