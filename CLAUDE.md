...
- On-device AI (2026-06-19): optional "Explain with AI" on password generator, **opt-in/off by default** (`enableOnDeviceAI` + `allowStrengthExplanation` in `src/core/ai/aiSettings.ts`, localStorage). Chrome built-in `LanguageModel` only — no remote/extension provider, no app-initiated model download (gated to `availability() === 'available'`). Only strength label + rounded entropy sent (never password/secret/username/origin); AI treated as outside ZK boundary, no prompt/response logged or persisted. `LanguageModel` access isolated in `src/core/ai/promptApi.ts`; secret-free prompt in `strengthExplain.ts`. See `SECURITY.md` §On-Device AI Boundary.
