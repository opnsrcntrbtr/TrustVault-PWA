...
- webauthn-prf-zktv refactor (2026-07-06): PRF ceremonies (`enrollPrfCredential`/`evaluatePrf`) and vault-key wrap/unwrap (`wrapSecret`/`unwrapSecret`/`serializeRecord`/`parseRecord`) now come from the [`webauthn-prf-zktv`](https://www.npmjs.com/package/webauthn-prf-zktv) npm package instead of inline code — same engine, extracted from this app, published as the paper's reference implementation. `src/core/auth/webauthn.ts` shrank to an app-specific thin layer (native-app gate, device naming, PRF detection re-exports); `src/core/auth/biometricVaultKey.ts` and the SimpleWebAuthn-based `registerBiometric`/`authenticateBiometric`/verify* functions were deleted, along with the `@simplewebauthn/*` dependencies. Pre-refactor `prf-v1` records (old HKDF label `'TrustVault Vault Key Wrapping v1'`, raw `EncryptedData` JSON) are migrated **lazily on next unlock** via the package's `fromTrustVaultRecord()` — one ceremony, no data loss. Master-password unlock path is unchanged.
